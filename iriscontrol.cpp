...
void IRIScontrol::OnButtonDisable()
{
    UpdateNodeIdString();
	BOOL Done1,Done2;
	Done1 = VCS_SetDisableState(m_KeyHandle, m_usNodeId, &m_ulErrorCode);
	Done2 = VCS_SetDisableState(m_KeyHandle, m_usNodeId2, &m_ulErrorCode);
    if(!Done1
		&& !Done2)
    {
        ShowErrorInformation(m_ulErrorCode);
...
void IRIScontrol::OnButtonMove()  
{
    ...
    {
		BOOL Done1, Done2;
		Done1 = VCS_MoveToPosition(m_KeyHandle, m_usNodeId, m_lTargetPosition, m_oRadio, m_oImmediately, &m_ulErrorCode);
		Done2 = VCS_MoveToPosition(m_KeyHandle, m_usNodeId2, m_lTargetPosition2, m_oRadio, m_oImmediately, &m_ulErrorCode);
		if(!Done1 
			&& !Done2)
        {
            ShowErrorInformation(m_ulErrorCode);
